...
@interface MCZXingVC ()<ZXCaptureDelegate,MCZBarViewDelegate>{
...
@property (nonatomic, strong)  MCZBarView *scanRectView;

@end

@implementation MCZXingVC
...
-(void)initView{
    ...
    _scanRectView=[[MCZBarView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)];
...
- (void)captureResult:(ZXCapture *)capture result:(ZXResult *)result {
    ...
    self.scanRectView.labIntroudction.text=result.text;
    // Vibrate
    ...
    if (_delegate) {
        [_delegate readResults:result.text];
        [self onClickBack];
    }
